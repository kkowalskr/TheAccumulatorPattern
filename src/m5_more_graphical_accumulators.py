...
def draw_squares_from_circle(n, circle, window):
    ...
    for k in range(n):
        point = rg.Point(circle.center.x + circle.radius * k,
                         circle.center.y + circle.radius * k)
        squares = rg.Square(point, 2 * circle.radius)
...
def run_test_draw_circles_from_rectangle():
    ...
    title = title + '8 blue in a row, 3 in column, and 5 in column, 4 in row'
...
def draw_circles_from_rectangle(m, n, rectangle, window):
    ...
    for k in range(m):
        circle = rg.Circle(rg.Point(rectangle.get_lower_left_corner().x -
                                    0.5 * rectangle.get_height() -
